{% macro require_user_context() %}
    ...
    {% if execute and flags.WHICH in ['run', 'build', 'test'] and (user_uuid is none or run_uuid is none or user_uuid | string | trim == '' or run_uuid | string | trim == '') %}
        {{ exceptions.raise_compiler_error(
            "user_refresh requires non-empty user_uuid and run_uuid vars"
        ) }}
    {% endif %}
    {% if execute and flags.WHICH in ['run', 'build', 'test'] %}
        {% for field_name, raw_value in [('user_uuid', user_uuid), ('run_uuid', run_uuid)] %}
