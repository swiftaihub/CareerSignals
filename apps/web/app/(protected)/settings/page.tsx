...
export default function SettingsPage() {
  ...
  const activeRun = useMemo(() => runs.find((run) => run.status === "queued" || run.status === "running"), [runs]);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(async () => {
      try {
        ...
        if (next.status !== "queued" && next.status !== "running") await refreshRuns();
      } catch (requestError) {
  ...
  async function startPipeline() {
    ...
    try {
      ...
      setMessage("Your personal dbt refresh was queued. Shared job sources were not contacted.");
  ...
  return (
    <AppShell>
      ...
      {freshness ? <SectionCard className="mt-6" title="Shared job-data freshness" description="CareerSignals refreshes source job data automatically. There is no user-facing Connector refresh control."><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Status" value={freshness.overall.status} /><MetricCard label="Last Refresh" value={formatDateTime(freshness.overall.last_successful_refresh_at)} /><MetricCard label="Next Scheduled" value={formatDateTime(freshness.overall.next_scheduled_refresh_at)} /><MetricCard label="Data As Of" value={formatDateTime(freshness.overall.data_as_of)} /></div>{freshness.overall.is_stale ? <p className="mt-4 flex gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><AlertTriangle className="h-4 w-4" />Shared job data may be stale.</p> : null}<div className="mt-4 table-shell overflow-x-auto"><table className="data-table min-w-[700px]"><thead><tr><th>Source</th><th>Status</th><th>Last successful refresh</th><th>Records retained</th><th>Message</th></tr></thead><tbody>{freshness.sources.map((source) => <tr key={source.source_name}><td className="font-semibold">{source.source_name}</td><td>{source.status}</td><td>{formatDateTime(source.last_successful_refresh_at)}</td><td>{source.records_retained ?? 0}</td><td>{source.public_status_message || "—"}</td></tr>)}</tbody></table></div></SectionCard> : null}
      ...
      <SectionCard className="mt-6" title="Personal dbt Pipeline" description="This refresh scores and summarizes the existing shared job universe using your immutable configuration snapshot. It never calls Adzuna, SerpApi, Greenhouse, Lever, USAJOBS, or any external job API.">
        <div className="mb-5 grid gap-4 sm:grid-cols-3"><MetricCard label="Latest Status" value={runs[0]?.status || "Never run"} /><MetricCard label="Last Successful Result" value={lastSuccessfulRun ? formatDateTime(lastSuccessfulRun.published_at || lastSuccessfulRun.completed_at) : "None"} /><MetricCard label="Last Successful Run" value={user?.last_successful_pipeline_run_uuid || "None"} /></div>
